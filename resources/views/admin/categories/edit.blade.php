...
<div class="max-w-3xl mx-auto">
    ...
    {{-- Title --}}
    <h1 class="text-4xl font-display tracking-tight mb-10">Редагувати категорію</h1>
    ...
    <form action="{{ route('admin.categories.update', $category) }}" method="POST">
        ...
        <div class="mb-8">
            ...
            @error('name') 
                <p class="text-brand-accent text-xs font-mono">{{ $message }}</p> 
            @enderror
        </div>
    ...
    @enderror
        ...
        <div class="mb-8">
            ...
            <textarea name="description" 
                      rows="3"
                      class="w-full bg-transparent border-b border-brand-black/40 py-2 focus:border-brand-black">{{ old('description', $category->description) }}</textarea>
            @error('description') 
                <p class="text-brand-accent text-xs font-mono">{{ $message }}</p> 
            @enderror
        </div>
